...
export class RequestsComponentTenantMoveOut implements OnInit {
  ...
  async ngOnInit() {
    ...
    var adminReqDataSession = JSON.parse(
      sessionStorage.getItem("tenant-move-out-reqs-session")
    );
    ...
    if (adminReqDataSession != null) {
      ...
      sessionStorage.setItem(
        "tenant-move-out_reqs_fetched_time",
    ...
    var diff =
      now -
      Number(JSON.parse(sessionStorage.getItem("tenant-move-out_reqs_fetched_time")));

    if (diff >= 10) {
      // this.isLoading = true;
      this.isContentLoading = true;
      this.clearAllVariables();
      sessionStorage.removeItem("tenant-move-out_reqs_fetched_time");
      sessionStorage.removeItem("tenant-move-out-reqs-session");
      await this.initFunction(user[0]["id"], user[0]["auth_type"]);
      sessionStorage.setItem(
        "tenant-move-out_reqs_fetched_time",
        JSON.stringify(new Date().getMinutes())
  ...
  async initFunction(userId, auth) {
    this.apiService
      .getUserTenantMoveOutRequests(userId)
      .subscribe((va: any[]) => {
        this.allRequests = va;
        console.log(va);
      });
