...
export class RequestsComponentConditioning implements OnInit {
  ...
  fetchData() {
    ...
    var adminReqDataSession = JSON.parse(
      sessionStorage.getItem("conditioning_reqs_session")
    );
  ...
  async ngOnInit() {
    ...
    var previous = JSON.parse(sessionStorage.getItem("conditioning_reqs_fetched_time"));
    ...
    if (previous != null) {
      ...
      if (diff >= 5) {
        sessionStorage.removeItem("conditioning_reqs_session");
        this.fetchData();
      } else {
        if (adminReqDataSession != null) {
          this.allRequests = adminReqDataSession;
