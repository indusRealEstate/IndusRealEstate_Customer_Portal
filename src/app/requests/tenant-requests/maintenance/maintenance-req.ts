...
@Component({
  selector: "app-requests-maintenance",
  templateUrl: "./maintenance-req.html",
  styleUrls: ["./maintenance-req.scss"],
})
export class RequestsComponentMaintenance implements OnInit {
  ...
  refreshTable() {
    var userData = localStorage.getItem("currentUser");
    var user = JSON.parse(userData);
    sessionStorage.removeItem("maintenance-reqs-session");
    this.isContentLoading = true;

    this.apiService
      .getUserMaintenanceRequests(user[0]["id"])
      .subscribe((va: any[]) => {
        this.allRequests = va;
  ...
  async ngOnInit() {
    ...
    var adminReqDataSession = JSON.parse(
      sessionStorage.getItem("maintenance-reqs-session")
    );
    ...
    var diff =
      now -
      Number(
        JSON.parse(sessionStorage.getItem("maintenance_reqs_fetched_time"))
      );
    ...
    if (diff >= 10) {
      ...
      sessionStorage.removeItem("maintenance_reqs_fetched_time");
      sessionStorage.removeItem("maintenance-reqs-session");
      this.apiService
        .getUserMaintenanceRequests(user[0]["id"])
        .subscribe((va: any[]) => {
