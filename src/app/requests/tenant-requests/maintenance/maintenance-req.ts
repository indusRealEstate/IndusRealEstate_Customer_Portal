...
export class RequestsComponentMaintenance implements OnInit {
  ...
  fetchData() {
    ...
    var adminReqDataSession = JSON.parse(
      sessionStorage.getItem("maintenance_reqs_session")
    );
  ...
  async ngOnInit() {
    ...
    var previous = JSON.parse(
      sessionStorage.getItem("maintenance_reqs_fetched_time")
    );
    ...
    if (previous != null) {
      ...
      if (diff >= 5) {
        sessionStorage.removeItem("maintenance_reqs_session");
        this.fetchData();
      } else {
        if (adminReqDataSession != null) {
          this.allRequests = adminReqDataSession;
