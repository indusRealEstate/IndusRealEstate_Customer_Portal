...
export class RequestsComponentTenantMoveIn implements OnInit {
  ...
  fetchData() {
    ...
    var adminReqDataSession = JSON.parse(
      sessionStorage.getItem("move_in_reqs_session")
    );
        ...
        .add(() => {
          ...
          if (this.allRequests.length != 0) {
            sessionStorage.setItem(
              "move_in_reqs_session",
              JSON.stringify(this.allRequests)
            );
          }
        });
      sessionStorage.setItem(
        "move_in_reqs_fetched_time",
  ...
  async ngOnInit() {
    ...
    var previous = JSON.parse(
      sessionStorage.getItem("move_in_reqs_fetched_time")
    );
    ...
    if (previous != null) {
      ...
      if (diff >= 5) {
        sessionStorage.removeItem("move_in_reqs_session");
        this.fetchData();
      } else {
        if (adminReqDataSession != null) {
          this.allRequests = adminReqDataSession;
