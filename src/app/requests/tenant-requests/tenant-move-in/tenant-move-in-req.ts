...
@Component({
  selector: "app-requests-tenant-move-in",
  templateUrl: "./tenant-move-in-req.html",
  styleUrls: ["./tenant-move-in-req.scss"],
})
export class RequestsComponentTenantMoveIn implements OnInit {
  ...
  constructor(
    ...
    if (user[0]["auth_type"] != "admin") {
      this.route.queryParams.subscribe((e) => {
        if (e == null) {
          router.navigate([`/tenant-move-in-requests`], {
  ...
  async ngOnInit() {
    ...
    var adminReqDataSession = JSON.parse(
      sessionStorage.getItem("tenant-move-in-reqs-session")
    );
    ...
    if (adminReqDataSession != null) {
      ...
      sessionStorage.setItem(
        "tenant-move-in_reqs_fetched_time",
    ...
    var diff =
      now -
      Number(JSON.parse(sessionStorage.getItem("tenant-move-in_reqs_fetched_time")));

    if (diff >= 10) {
      // this.isLoading = true;
      this.isContentLoading = true;
      this.clearAllVariables();
      sessionStorage.removeItem("tenant-move-in_reqs_fetched_time");
      sessionStorage.removeItem("tenant-move-in-reqs-session");
      await this.initFunction(user[0]["id"], user[0]["auth_type"]);
      sessionStorage.setItem(
        "tenant-move-in_reqs_fetched_time",
        JSON.stringify(new Date().getMinutes())
  ...
  async initFunction(userId, auth) {
    this.apiService
      .getUserTenantMoveInRequests(userId)
      .subscribe((va: any[]) => {
        this.allRequests = va;
        console.log(va);
      });
