...
export class AppointmentsComponent implements OnInit {
        ...
        this.calendarOptions = {
          ...
          eventClick: (e) => {
            for (let ev of this.rawEventData) {
              if (ev["event_name"] == e.event.title) {
                var dateDay = Number(
                  e.event.start.toISOString().split("T")[0].split("-")[2]
                );
                var currentDate =
                  e.event.start.toISOString().split("T")[0].split("-")[0] +
                  "-" +
                  e.event.start.toISOString().split("T")[0].split("-")[1] +
                  "-" +
                  dateDay.toString();
          ...
          dateClick: (day) => {
            ...
            if (today <= day.date.getTime()) {
              ...
              var dateDay =
                Number(day.date.toISOString().split("T")[0].split("-")[2]) + 1;
              var currentDate =
                day.date.toISOString().split("T")[0].split("-")[0] +
                "-" +
                day.date.toISOString().split("T")[0].split("-")[1] +
                "-" +
                dateDay.toString();
  ...
  getCurrentMonthEvents() {
    ...
    sessionStorage.setItem(
      "currentMonthEvents",
      JSON.stringify(this.currentMonthEvents)
    );
  }
  ...
  getUserAppointments() {
    ...
    this.apiService.getUserAppoinments(userId).subscribe((data: any[]) => {
      if (data != null) {
        ...
        for (let e of data) {
          var eventMonth = this.getMonthByNumber(
  ...
  openModalDate(dateString: any, events: any[], date: any) {
    ...
    this.modalDialogDate = this.matDialog.open(
      ModalComponentDate,
      this.dialogConfig
    );
