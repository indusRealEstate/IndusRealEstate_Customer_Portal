...
@Component({
  selector: "rejected-request-tab",
  templateUrl: "./rejected-request.html",
  styleUrls: ["./rejected-request.scss"],
})
export class AdminRequestsRejected implements OnInit {
  ...
  fetchData() {
    var adminReqDataSession = JSON.parse(
      sessionStorage.getItem("admin_rejected_requests_session")
    );
    if (adminReqDataSession != null) {
      ...
      this.adminService
        ...
        .subscribe((va: any[]) => {
          console.log(va);
          var final_data = va.filter((req)=> req.request_status == 'rejected')
        ...
        .add(() => {
          ...
          if (this.allRequests.length != 0) {
            sessionStorage.setItem(
              "admin_rejected_requests_session",
              JSON.stringify(this.allRequests)
            );
          }
        });
      sessionStorage.setItem(
        "admin_rejected_requests_session_time_admin",
  ...
  async ngOnInit() {
    ...
    var previous = JSON.parse(
      sessionStorage.getItem("admin_rejected_requests_session_time_admin")
    );
    ...
    if (previous != null) {
      ...
      if (diff >= 5) {
        sessionStorage.removeItem("admin_rejected_requests_session");
        this.fetchData();
      } else {
        if (adminReqDataSession != null) {
          this.allRequests = adminReqDataSession;
