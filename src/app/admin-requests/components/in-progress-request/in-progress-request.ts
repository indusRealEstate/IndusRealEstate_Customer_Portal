...
@Component({
  selector: "in-progress-request-tab",
  templateUrl: "./in-progress-request.html",
  styleUrls: ["./in-progress-request.scss"],
})
export class AdminRequestsInProgress implements OnInit {
  ...
  fetchData() {
    var adminReqDataSession = JSON.parse(
      sessionStorage.getItem("admin_inprogress_requests_session")
    );
    if (adminReqDataSession != null) {
      ...
      this.adminService
        ...
        .subscribe((va: any[]) => {
          console.log(va);
          var final_data = va.filter((req)=> req.request_status == 'inprogress')
        ...
        .add(() => {
          ...
          if (this.allRequests.length != 0) {
            sessionStorage.setItem(
              "admin_inprogress_requests_session",
              JSON.stringify(this.allRequests)
            );
          }
        });
      sessionStorage.setItem(
        "admin_inprogress_requests_session_time_admin",
  ...
  async ngOnInit() {
    ...
    var previous = JSON.parse(
      sessionStorage.getItem("admin_inprogress_requests_session_time_admin")
    );
    ...
    if (previous != null) {
      ...
      if (diff >= 5) {
        sessionStorage.removeItem("admin_inprogress_requests_session");
        this.fetchData();
      } else {
        if (adminReqDataSession != null) {
          this.allRequests = adminReqDataSession;
