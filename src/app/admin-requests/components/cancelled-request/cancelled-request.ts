...
@Component({
  selector: "cancelled-request-tab",
  templateUrl: "./cancelled-request.html",
  styleUrls: ["./cancelled-request.scss"],
})
export class AdminRequestsCancelled implements OnInit {
  ...
  fetchData() {
    var adminReqDataSession = JSON.parse(
      sessionStorage.getItem("admin_cancelled_requests_session")
    );
    if (adminReqDataSession != null) {
      ...
      this.adminService
        ...
        .subscribe((va: any[]) => {
          // console.log(va);
          var final_data = va.filter((req)=> req.request_status == 'cancelled')
        ...
        .add(() => {
          ...
          if (this.allRequests.length != 0) {
            sessionStorage.setItem(
              "admin_cancelled_requests_session",
              JSON.stringify(this.allRequests)
            );
          }
        });
      sessionStorage.setItem(
        "admin_cancelled_requests_session_time_admin",
  ...
  async ngOnInit() {
    ...
    var previous = JSON.parse(
      sessionStorage.getItem("admin_cancelled_requests_session_time_admin")
    );
    ...
    if (previous != null) {
      ...
      if (diff >= 5) {
        sessionStorage.removeItem("admin_cancelled_requests_session");
        this.fetchData();
      } else {
        if (adminReqDataSession != null) {
          this.allRequests = adminReqDataSession;
