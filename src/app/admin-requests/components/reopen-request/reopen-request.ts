...
@Component({
  selector: "reopen-request-tab",
  templateUrl: "./reopen-request.html",
  styleUrls: ["./reopen-request.scss"],
})
export class AdminRequestsReOpenTab implements OnInit {
  ...
  fetchData() {
    var adminReqDataSession = JSON.parse(
      sessionStorage.getItem("admin_reopen_requests_session")
    );
    if (adminReqDataSession != null) {
      ...
      this.adminService
        ...
        .subscribe((va: any[]) => {
          console.log(va);
          var final_data = va.filter((req)=> req.request_status == 'reopen')
        ...
        .add(() => {
          ...
          if (this.allRequests.length != 0) {
            sessionStorage.setItem(
              "admin_reopen_requests_session",
              JSON.stringify(this.allRequests)
            );
          }
        });
      sessionStorage.setItem(
        "admin_reopen_requests_session_time_admin",
  ...
  async ngOnInit() {
    ...
    var previous = JSON.parse(
      sessionStorage.getItem("admin_reopen_requests_session_time_admin")
    );
    ...
    if (previous != null) {
      ...
      if (diff >= 5) {
        sessionStorage.removeItem("admin_reopen_requests_session");
        this.fetchData();
      } else {
        if (adminReqDataSession != null) {
          this.allRequests = adminReqDataSession;
