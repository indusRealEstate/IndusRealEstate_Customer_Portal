...
export class AdminTenantClients implements OnInit {
  ...
  async ngOnInit() {
    ...
    var sessionData = sessionStorage.getItem("all_tenant_clients");

    if (sessionData != null) {
      this.allTenantClients = JSON.parse(sessionData);
              ...
              if (val == true) {
                ...
                setTimeout(() => {
                  ...
                  sessionStorage.setItem(
                    "all_tenant_clients",
                    JSON.stringify(this.allTenantClients)
                  );

                  console.log(this.allTenantClients);
                }, 2000);
              }
            });
          }, 1000);
  ...
  async getAllClients(userId) {
    this.adminService.getAllClients(userId).subscribe(async (e: Array<any>) => {
      ...
      for (let c of e) {
        i++;
        if (c.auth_type == "tenant") {
          this.allTenantClients.push(c);
        }
      }
  ...
  checkEveryDataPresent() {
    var dataLack = 0;
    for (let c of this.allTenantClients) {
