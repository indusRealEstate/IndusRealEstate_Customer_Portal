...
export class AdminRequestsSpam implements OnInit {
  ...
  fetchData() {
    var adminReqDataSession = JSON.parse(
      sessionStorage.getItem("admin_spam_requests_session")
    );
        ...
        .add(() => {
          ...
          if (this.allRequests.length != 0) {
            sessionStorage.setItem(
              "admin_spam_requests_session",
              JSON.stringify(this.allRequests)
            );
          }
        });
      sessionStorage.setItem(
        "admin_spam_requests_session_time_admin",
  ...
  async ngOnInit() {
    ...
    var previous = JSON.parse(
      sessionStorage.getItem("admin_spam_requests_session_time_admin")
    );
    ...
    if (previous != null) {
      ...
      if (diff >= 5) {
        sessionStorage.removeItem("admin_spam_requests_session");
        this.fetchData();
      } else {
        if (adminReqDataSession != null) {
          this.allRequests = adminReqDataSession;
