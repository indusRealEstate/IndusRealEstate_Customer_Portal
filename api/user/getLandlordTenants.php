...
if (isset($postdata) && !empty($postdata)) {
    ...
    foreach ($tenantIDS->tenant_ids as $key => $value) {
        $result = $dbase->execute("SELECT * FROM user u, user_details ud, user_properties usp WHERE u.id = '$value->user_id' AND ud.user_id = '$value->user_id' AND usp.property_id = '$value->property_id'");
